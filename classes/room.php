...
class room extends crud
{
    ...
    /**
     * Returns building name and room number (name)
     * @return void
     */
    public function get_full_name()
    {
        ...
        $sql = "
            ...
            FROM
                {order_campus} c Inner Join
                {order_building} b On c.code = b.campus_code Inner Join
                {order_floor} f On b.code = f.building_code Inner Join
                {order_room} r On f.id = r.floor_id Inner Join
                {order_room_type} rt On rt.id = r.room_type_id
            WHERE 
                r.id =?";

        $name = '';

        if ($result = $DB->get_record_sql($sql, [$this->id])) {
            $name = $result->code . ' ' . $result->shortname . $result->room;
        }
